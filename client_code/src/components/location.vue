            ...
            <div class="aMapMain">
                <div class="aMap">
                    <el-amap ref="mapRef" :min-zoom="8" :max-zoom="22" :center="center" :zoom="zoom"
                        ...
                        <el-amap-control-geolocation @complete="getLocation" @error="getLocationError" extensions="all" />
                    </el-amap>
                </div>
...
const formKey = ref('')
const mapShow = (key) => {
    if(formKey){    //如果有指定保存的位置modelValue[formKey]，则将后续选取的位置保存到其中
        formKey.value = key
    }
    initMapInfo()
...
const initMapInfo = () => {
    ...
    if(formKey.value){
        let arr = props.modelValue[formKey.value]?.split(',')
        if(arr.length==3){
...
const getLocationError = (e)=>{
    window.ElMessage.error("获取定位失败，详情查看浏览器控制台")
    console.log(e)
    console.log("请检查浏览器是否允许定位，登录高德开发者平台创建并替换系统中的高德key和密钥(main.js中)")
}
//点击描点
const clickMap = (e) => {
...
const chooseMapClick=()=>{
    ...
    if(formKey.value){
        props.modelValue[formKey.value] = `${thePosition.value.lng},${thePosition.value.lat},${thePosition.value.fulladdress}`
